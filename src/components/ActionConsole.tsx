            ...
            {!isActive && result ? (
               canCloseBattle ? (
                <div className="flex-1 flex gap-2">
                    ...
                    >
                        {result === 'win'
                            ? (activeRouteId && explorationStep < 5 ? <><span>ลุยต่อ (ด่าน {explorationStep + 1}/5)</span> <ArrowRightCircle size={16}/></> : '✅ ภารกิจสำเร็จ! (กลับบ้าน)')
                            : (result === 'fled' ? '💨 กลับบ้าน (หนีสำเร็จ)' : '💀 พ่ายแพ้... (กลับบ้าน)')}
                    </button>
                </div>
               ) : (
